...
RUN mkdir /target
ENV JAVA_OPTS="-javaagent:/tmp/jacoco-agent.jar=destfile=/target/jacoco.exec,append=true -Duser.timezone=UTC"

ADD https://github.com/ufoscout/docker-compose-wait/releases/download/2.2.1/wait /wait
RUN chmod +x /wait

## Launch the wait tool and then your application
CMD /wait && touch /target/jacoco.exec && chmod 666 /target/jacoco.exec && /tomcat/bin/catalina.sh run
